...
########################################################################
class StrategySimpleBreakFilter(CtaTemplate):
    """结合ATR和RSI指标的一个分钟线交易策略"""
    className = 'StrategySimpleBreakFilter'
    author = u'向律楷'
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(StrategySimpleBreakFilter, self).__init__(ctaEngine, setting)
    ...
    def onInit(self):
        ...
        # ！！手动调用父类实现
        super(StrategySimpleBreakFilter, self).onInit()
    ...
    def onStart(self):
        ...
        # ！！手动调用父类实现
        super(StrategySimpleBreakFilter, self).onStart()
    ...
    def onBar(self, bar):
        ...
        if self.pos == 0:
            ...
            # if max(self.closeArray[-self.shortPeriod:-2]) > max(self.closeArray[-self.longPeriod:-2]):  # 过滤器

                if self.closeArray[-1] > max(self.closeArray[-self.breakWindow:-2]): #突破breakWindow根k线股价
            ...
            # elif max(self.closeArray[-self.shortPeriod:-2]) < max(self.closeArray[-self.longPeriod:-2]):  # 过滤器
                if self.closeArray[-1] < min(self.closeArray[-self.breakWindow:-2]) :
...
if __name__ == '__main__':
    ...
    # 在引擎中创建策略对象
    setting = dict(vtSymbol='RB0000', inBacktesting=True, breakWindow=10, trailingStop=4.0,shortPeriod=30,longPeriod=100,
         stopLoss=1.2, klinePeriod=dre.ctaKLine.PERIOD_1DAY)
    engine.initStrategy(StrategySimpleBreakFilter,setting)  # 初始化策略
    ...
    #
    # # 多进程优化，耗时：89秒
    # resultList=engine.runParallelOptimization(StrategySimpleBreakFilter, setting)
