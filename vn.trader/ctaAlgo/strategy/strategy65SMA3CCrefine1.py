...
########################################################################
class Strategy65SMA3CCRefine1(CtaTemplate):
    """结合ATR和RSI指标的一个分钟线交易策略"""
    className = 'Strategy65SMA3CCRefine1'
    author = u'向律楷'

    # 策略参数
    trailingStop = 3.1  # 百分比移动止损，必须用浮点数
    stopLoss = 0.8  # 百分比固定止损，必须用浮点数
    profitLine = 1.6 # 百分比固定止损，必须用浮点数
    # period = [5, 14]  # EMA周期
    shortPeriod = 4  # 短周期
    longPeriod = 20  # 长周期
    RaviLimit = 0.3  # 过滤器
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(Strategy65SMA3CCRefine1, self).__init__(ctaEngine, setting)
    ...
    def onInit(self):
        ...
        # ！！手动调用父类实现
        super(Strategy65SMA3CCRefine1, self).onInit()
    ...
    def onStart(self):
        ...
        # ！！手动调用父类实现
        super(Strategy65SMA3CCRefine1, self).onStart()
    ...
    def onBar(self, bar):
        ...
        if self.pos == 0:
            ...
            # 计算多头移动止损
            if self.intraTradeHigh > self.longPrice * (1 + self.profitLine / 100.0): #盈利后上移止损位
                # longStop = max(self.intraTradeHigh * (1 - self.trailingStop / 100.0),
                #                self.longPrice + (self.intraTradeHigh - self.longPrice) * 0.5)
                longStop = max(self.intraTradeHigh * (1 - self.trailingStop / 100.0),
                               self.intraTradeHigh * (1 - 0.8 * self.profitLine / 100.0))
            else:
                longStop = max(self.intraTradeHigh * (1 - self.trailingStop / 100.0),
            ...
            self.intraTradeHigh = bar.high
            # 计算空头移动止损
            if self.intraTradeLow < self.shortPrice * (1 - self.profitLine / 100.0): #盈利后下移止损位
                # shortStop = min(self.intraTradeLow * (1 + self.trailingStop / 100.0),
                #                self.shortPrice + (self.intraTradeLow - self.shortPrice) * 0.5)
                shortStop = min(self.intraTradeLow * (1 + self.trailingStop / 100.0),
                                self.intraTradeLow * (1 + 0.8 * self.profitLine / 100.0))
            else:
                shortStop = min(self.intraTradeLow * (1 + self.trailingStop / 100.0),
...
if __name__ == '__main__':
    ...
    # 在引擎中创建策略对象
    engine.initStrategy(Strategy65SMA3CCRefine1,
                        dict(vtSymbol='RB0000TEST', inBacktesting=True, shortPeriod=6, longPeriod=55, trailingStop=3.1,
                             stopLoss=1.1, profitLine=2.2, RaviLimit=0.5, klinePeriod=dre.ctaKLine.PERIOD_30MIN))  # 初始化策略
    ...
    # setting.addParameter('trailingStop', 2.0, 5.0, 0.5)            # 增加一个固定数值的参数
    # setting.addParameter('stopLoss', 0.8, 2.0, 0.2)  # 增加一个固定数值的参数
    # setting.addParameter('profitLine', 1.0, 5.0, 0.5)  # 增加一个固定数值的参数
    # setting.addParameter('RaviLimit', 0.5, 0.5, 0.1)            # 增加一个固定数值的参数
    # setting.addParameter('inBacktesting', True)            # 增加一个固定数值的参数
    # setting.addParameter('vtSymbol', 'HC0000')            # 增加一个固定数值的参数
    ...
    #
    # # 多进程优化，耗时：89秒
    # engine.runParallelOptimization(Strategy65SMA3CCRefine1, setting)
    #
