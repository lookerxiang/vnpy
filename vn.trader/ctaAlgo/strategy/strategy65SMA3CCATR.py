...
########################################################################
class Strategy65SMA3CCATR(CtaTemplate):
    """结合ATR和RSI指标的一个分钟线交易策略"""
    className = 'Strategy65SMA3CCATR'
    author = u'向律楷'
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(Strategy65SMA3CCATR, self).__init__(ctaEngine, setting)
    ...
    def onInit(self):
        ...
        # ！！手动调用父类实现
        super(Strategy65SMA3CCATR, self).onInit()
    ...
    def onStart(self):
        ...
        # ！！手动调用父类实现
        super(Strategy65SMA3CCATR, self).onStart()
    ...
    def onBar(self, bar):
        ...
        if self.pos == 0:
            ...
            if self.closeArray[-1] > self.longArray[-1] and self.closeArray[-2] > self.longArray[-2] and \
                ...
                if self.closeArray[-1] > self.shortArray[-1] > self.shortArray[-2] and self.closeArray[-1] >self.closeArray[-2]  :
...
if __name__ == '__main__':
    ...
    # 在引擎中创建策略对象
    engine.initStrategy(Strategy65SMA3CCATR,
                        dict(vtSymbol='RB0000', inBacktesting=True, shortPeriod=8, longPeriod=16, trailingStop=3.2,
                             stopLoss=1.2, RaviLimit=0.2, klinePeriod=dre.ctaKLine.PERIOD_1DAY))  # 初始化策略
    ...
    # setting.addParameter('shortPeriod', 20, 20, 2)    # 增加第一个优化参数atrLength，起始11，结束12，步进1
    # setting.addParameter('longPeriod', 40, 40, 2)        # 增加第二个优化参数atrMa，起始20，结束30，步进1
    # setting.addParameter('trailingStop', 1.0, 8.0, 0.5)            # 增加一个固定数值的参数
    # setting.addParameter('stopLoss', 0.4, 3.0, 0.2)  # 增加一个固定数值的参数
    # setting.addParameter('RaviLimit', 0.0, 0.0, 0.1)            # 增加一个固定数值的参数
    ...
    #
    # # 多进程优化，耗时：89秒
    # resultList=engine.runParallelOptimization(Strategy65SMA3CCATR, setting)
