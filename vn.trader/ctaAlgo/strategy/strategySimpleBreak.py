...
class StrategySimpleBreak(CtaTemplate):
    ...
    bar = None  # K线对象
    barMinute = ''  # K线当前的分钟
    bufferSize = 80  # 需要缓存的数据的量，应该小于等于初始化数据所用的天数里的数据量，以使得程序可以立即进入交易状态。
    ...
    def onInit(self):
        """初始化策略（必须由用户继承实现）"""
        self.writeCtaLog(u'StrategySimpleBreak策略初始化')
        ...
        if self.klinePeriod == dre.ctaKLine.PERIOD_1DAY:
            startDatetime -= dt.timedelta(hours=16)

        self.startHistoryData(self.bufferSize)
        initData = self.getLastKlines(self.intiWindow, period=self.klinePeriod, from_datetime=startDatetime)
        for bar in initData:
            self.updateData(bar)
        self.endHistoryData()
    ...
    def onStart(self):
        """启动策略（必须由用户继承实现）"""
        self.writeCtaLog(u'StrategySimpleBreak策略启动')
            ...
            if dir:  # 有仓位
                ...
                if targetTrade:
                    ...
                    cursor = col.find(filter={'datetime': {'$gte': targetTrade['tradeDatetime'],
                                                           '$lte': dt.datetime.now()+dt.timedelta(hours=6)}},
                                      projection={'_id': False},
                    ...
                    for kline in cursor:
                        ...
                        last_kline = kline
                    if last_kline:
                        last_kline_obj = dre.ctaKLine.KLine(None)
                        last_kline_obj.__dict__.update(last_kline)
                        self.onBar(last_kline_obj)
    ...
    def updateData(self, bar):
        ...
        if len(lastKLines) < self.intiWindow:
            return

        # 计算指标数值

        self.ATR = min(talib.ATR(self.highArray, self.lowArray, self.closeArray, timeperiod=14)[-1],50) #平均真实波幅,最大值为40
    ...
    def onBar(self, bar):
        ...
        if self.pos == 0:
            ...
            print 'closeArray[-self.breakWindow:-1]  =>', self.closeArray[-self.breakWindow:-1]

            # 向上突破breakWindow根k线股价，开多。
            # if self.closeArray[-1] > self.longArray[-1]:
            if self.closeArray[-1] > max(self.closeArray[-self.breakWindow:-1]): #突破breakWindow根k线股价
                orderID = self.buy(bar.close + 10, 1)
                self.orderList.append(orderID)
                self.longPrice = bar.close  # 记录开仓价格，用于固定止损
                #用于发单那根k线的止损，防止当根k线发生重大损失
                longStop = self.longPrice - self.stopLoss * self.ATR
            ...
            if self.closeArray[-1] < min(self.closeArray[-self.breakWindow:-1]) :
                orderID = self.short(bar.close - 10, 1)
                self.orderList.append(orderID)
                self.shortPrice = bar.close  # 记录开仓价格，用于固定止损
                # 用于发单那根k线的止损，防止当根k线发生重大损失
                shortStop = self.shortPrice + self.stopLoss * self.ATR
           ...
           # 发出本地止损委托，并且把委托号记录下来，用于后续撤单
            orderIDs = self.sell(longStop, 1, stop=True)
            self.orderList.extend(orderIDs)
            print u'停止单号及价格：', orderIDs, longStop
            ...
            orderIDs = self.cover(shortStop, 1, stop=True)
            self.orderList.extend(orderIDs)
            print u'停止单号及价格：', orderIDs, shortStop
...
if __name__ == '__main__':
    ...
    # 在引擎中创建策略对象
    setting = dict(vtSymbol='RB0000', inBacktesting=True, breakWindow=5, trailingStop=3.2,
    ...
    # setting.addParameter('breakWindow', 5, 5, 1)    # 增加第一个优化参数atrLength，起始11，结束12，步进1
    # # setting.addParameter('filterPeriod', 10, 100, 2)        # 增加第二个优化参数atrMa，起始20，结束30，步进1
    # setting.addParameter('trailingStop', 3.2, 3.2, 0.2)            # 增加一个固定数值的参数
    # setting.addParameter('stopLoss', 1.0, 5.0, 0.1)  # 增加一个固定数值的参数
