...
class Strategy65SMA3CCRefine(CtaTemplate):
    ...
    def updateData(self, bar):
        ...
        self.ATR = min(talib.ATR(self.highArray, self.lowArray, self.closeArray, timeperiod=14)[-1],
                       50)  # 平均真实波幅,最大值为40
    ...
    def onBar(self, bar):
        ...
        if self.pos == 0:
            ...
            if self.closeArray[-1] > self.longArray[-1] and self.closeArray[-2] > self.longArray[-2] and \
                ...
                self.buyTrace = bar.high #突破最新k线的最高价时买入
                self.buyStop = min(self.lowArray[-1],self.lowArray[-2]) #止损价取最新k线及前一根k线的最低价的最小值
                if self.shortArray[-1] < self.shortArray[-2]:
                    if self.Ravi > self.RaviLimit:
                        orderID = self.buy(self.buyTrace, 1, stop=True)
            ...
            # 长短均线均向下，形成死叉或股价下穿短期均线卖出开空仓
                            ...
                            self.closeArray[-3] < self.longArray[-3]:
                self.shortTrace = bar.low #突破最新k线的最低价时买入
                self.shortStop = max(self.highArray[-1],self.highArray[-2]) #止损价取最新k线及前一根k线的最高价的最大值
                if self.shortArray[-1] > self.shortArray[-2]:
                    if self.Ravi > self.RaviLimit:
                        orderID = self.short(self.shortTrace, 1, stop=True)
                        self.orderList.append(orderID)
...
if __name__ == '__main__':
    ...
    engine.initStrategy(Strategy65SMA3CCRefine,
                        dict(vtSymbol='RB0000TEST', inBacktesting=True, shortPeriod=2, longPeriod=100, trailingStop=3.1,
                             stopLoss=0.8, RaviLimit=0.0, klinePeriod=dre.ctaKLine.PERIOD_30MIN))  # 初始化策略
