...
########################################################################
class Strategy65SMA3CCATR1(CtaTemplate):
    """结合ATR和RSI指标的一个分钟线交易策略"""
    className = 'Strategy65SMA3CCATR1'
    author = u'向律楷'

    # 策略参数
    trailingStop = 3.0  # ATR止损，必须用浮点数
    stopLoss = 1.5  # 百分比固定止损，必须用浮点数
    ...
    def __init__(self, ctaEngine, setting):
        """Constructor"""
        super(Strategy65SMA3CCATR1, self).__init__(ctaEngine, setting)
    ...
    def onInit(self):
        ...
        # ！！手动调用父类实现
        super(Strategy65SMA3CCATR1, self).onInit()
    ...
    def onStart(self):
        ...
        # ！！手动调用父类实现
        super(Strategy65SMA3CCATR1, self).onStart()
    ...
    def updateData(self, bar):
        ...
        self.ATR = talib.ATR(self.highArray, self.lowArray, self.closeArray, timeperiod=14)[-1] #平均真实波幅
    ...
    def onBar(self, bar):
        ...
        if self.pos == 0:
            ...
            # 计算多头移动止损
            if self.intraTradeHigh > self.longPrice * + self.profitLine*self.ATR: #盈利后上移止损位
                # longStop = max(self.intraTradeHigh * (1 - self.trailingStop / 100.0),
                #                self.longPrice + (self.intraTradeHigh - self.longPrice) * 0.5)
                longStop = max(self.intraTradeHigh - self.trailingStop *self.ATR,
                               self.intraTradeHigh - 0.5 * self.profitLine*self.ATR)
            else:
                longStop = max(self.intraTradeHigh - self.trailingStop *self.ATR,
            ...
            self.intraTradeHigh = bar.high
            # 计算空头移动止损
            if self.intraTradeLow < self.shortPrice  - self.profitLine * self.ATR: #盈利后下移止损位
                # shortStop = min(self.intraTradeLow * (1 + self.trailingStop / 100.0),
                #                self.shortPrice + (self.intraTradeLow - self.shortPrice) * 0.5)
                shortStop = min(self.intraTradeLow  + self.trailingStop * self.ATR,
                                self.intraTradeLow  + 0.5 * self.profitLine * self.ATR)
            else:
                shortStop = min(self.intraTradeLow  + self.trailingStop * self.ATR,
...
if __name__ == '__main__':
    ...
    # 在引擎中创建策略对象
    engine.initStrategy(Strategy65SMA3CCATR1,
                        dict(vtSymbol='RB0000', inBacktesting=True, shortPeriod=6, longPeriod=55, trailingStop=5.0,
                             stopLoss=1.0, profitLine=4.0, RaviLimit=0.5, klinePeriod=dre.ctaKLine.PERIOD_30MIN))  # 初始化策略

    # 设置引擎的回测模式为K线
    engine.setBacktestingMode(engine.BAR_MODE)

    # 设置回测用的数据起始日期
    engine.setStartDate('20151130', initDays=10)
    ...
    setting.addParameter('shortPeriod', 6, 6, 2)    # 增加第一个优化参数atrLength，起始11，结束12，步进1
    setting.addParameter('longPeriod', 55, 55, 5)        # 增加第二个优化参数atrMa，起始20，结束30，步进1
    setting.addParameter('trailingStop', 1.0, 8.0, 0.5)            # 增加一个固定数值的参数
    setting.addParameter('stopLoss', 0.8, 2.0, 0.2)  # 增加一个固定数值的参数
    setting.addParameter('profitLine', 1.0, 8.0, 0.5)  # 增加一个固定数值的参数
    ...
    # 多进程优化，耗时：89秒
    resultList=engine.runParallelOptimization(Strategy65SMA3CCATR1, setting)
    engine.runSurface('trailingStop','stopLoss',resultList)
